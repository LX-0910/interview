...
- <details><summary>十六进制</summary>
  ...
  >
  >       ```
  >       Color color = new Color32(0xFF, 0x00, 0x00, 0xFF); // 红色
  >       或者：
  >       ColorUtility.TryParseHtmlString("#FF0000", out var color);
  >       ```
  >       RGBA 本质就是 4 个字节（0–255）
  >
  >   3. 内存 / 数据结构 / 底层调试
  >
  >       ```
  ...
  >
  >       ```
  >       const int VisibleMask = 0x01;
  >       const int EnableMask  = 0x02;
  >       ```
  >       用于：
  >       - 权限控制
  >       - 状态组合
...
- <details><summary>二进制储存负数原理</summary>
...
- <details><summary>二进制储存小数原理</summary>
  </details>

- <details><summary>位移动</summary>
  
    > - **右移**：C# 中有两种“右移”语义（虽然语法上只有一种）：
    >     ```
    ...
    >   ↓
    >   右移30位，空出来的高位补1：11111111 11111111 11111111 11111111
    >   ↓
    >   结果：-1
    >   ```
    ...
    >   但符号位被改了
    >   所以：
    >   1 << 31 会变负数，不是逻辑错误，是位模式变化
    
    > - 超级常见技巧（Unity / 底层常用）:获取符号位
    >         
    >   ```
    >   int sign = x >> 31;
    >
...
- <details><summary>二进制/十进制/十六进制对比</summary>
  
  >- **二进制、十六进制 VS 十进制**：前者更接近底层，性能更高
  >---
  >- **二进制 VS 十六进制**：
  >   - 可读性：二进制直观，比如1 << n，表示第n位；十六进制更简洁，比如0x1，表示"值"
  >   - 使用场景：
  >
  >    | 使用场景 | 二进制 | 十六进制 |
  >    |---|---|---|
  >    |位标记（推荐二进制）| ```1<<n，更合适```  <br><br>[Flags]<br>enum State<br>{<br>&nbsp;&nbsp;&nbsp;&nbsp;None&nbsp;&nbsp;= 1 << 0,<br>&nbsp;&nbsp;&nbsp;&nbsp;Move &nbsp;= 1 << 1,<br>&nbsp;&nbsp;&nbsp;&nbsp;Jump&nbsp;&nbsp;= 1 << 2,<br>&nbsp;&nbsp;&nbsp;&nbsp;Attack = 1 << 3<br>}| ```不推荐十六进制``` <br><br>None&nbsp;&nbsp;= 0x0,<br>Move &nbsp;= 0x1,<br>Jump &nbsp;= 0x2,<br>Attack = 0x4(容易写错成0x3)<br><br>问题：<br>容易写错（0x3 这种 bug）<br>不直观“第几位”|
  >    | 掩码组合（两种都可以） | ```int mask2 = (1<<4)-1; // 00001111``` <br>逻辑清晰（前4位）| ```int mask = 0x0F;     // 00001111```<br>直观（已知值） |
  >    |底层/协议/颜色（推荐 16进制）| ```(1<<24) \| (1<<16) \| ...```<br>可读性爆炸 | int color = 0xFF00FF;|
  >    |动态位操作（必须用 1 << n）| int bitIndex = 5;<br>int mask = 1 << bitIndex;<br><br>16进制做不到这一点|不适合|
  >
  >   - **总结：本质区别**：
  >     - 二进制：1 << n → 过程导向（第几位）
  >     - 十六进制：0x1 → 结果导向（值是多少）
  ...
  >   - **项目规范**：
  >     - 位标记统一写法：1 << n
  >     - 固定值 / 配置 / 协议：0xFF / 0x1A / 0xDEADBEEF
  >     - 混合用法（高级写法）:const int Mask = (1 << 3) | (1 << 5); // 第3和第5位
  </details>
...
## 1.2 Unity
### 1.2.1 基础
- <details><summary>Mono生命周期函数</summary>
  </details>
...
为什么对象池可以减少GC，本质原因是什么？
对象池什么情况下会增加GC压力？
Unity中哪些对象最适合做对象池？

为什么LINQ会产生GC，具体哪些地方？
闭包为什么会产生GC？
为什么foreach不会产生GC，LINQ会产生？
LINQ一定不能用吗，什么时候可用？

为什么SetActive会导致Canvas.Rebuild？本质原因是什么？
canvas.rebuild分几个阶段？
为什么动静分离可以减少canvas.rebuild，本质原因是什么？
有没有不触发rebuild的操作？

实战：优化ScrollView滑动卡顿，怎么分析？怎么优化？
